<div class="wrap">
    ...
    <form method="post" action="options.php">
        ...
        <table class="form-table">
            <tr valign="top">
                ...
                <td><input type="text" name="realiza_ml_app_id" value="<?php echo esc_attr( get_option('realiza_ml_app_id') ); ?>" /></td>
            </tr>
            <tr valign="top">
                <th scope="row">Secret Key</th>
                <td><input type="password" name="realiza_ml_secret_key" value="<?php echo esc_attr( get_option('realiza_ml_secret_key') ); ?>" /></td>
            </tr>
        </table>
    ...
    if ( $access_token ) {
        ...
        if ( $app_id ) {
            ...
            $auth_url = "https://auth.mercadolivre.com.br/authorization?response_type=code&client_id={$app_id}&redirect_uri={$redirect_uri}";
            echo '<a href="' . esc_url( $auth_url ) . '" class="button button-primary">Conectar com Mercado Livre</a>';
            echo '<p class="description">Certifique-se de que a Redirect URI no seu App do Mercado Livre está configurada para: <code>' . $redirect_uri . '</code></p>';
        } else {
